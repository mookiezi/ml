...
def process_line(line):
    ...
    The function tags the line with <GOOD> or <BAD> based on its content.
    It doesn't modify the target field, just the input text.
    """
    cleaned_line = clean_line(line)
    if cleaned_line:
        # Only add the tag to the input (not the target)
        label = "<GOOD>" if not is_bad_message(cleaned_line) else "<BAD>"  # Changed tags here
...
def process_file(input_file, output_file_tagged, output_file_good_only, max_workers=4):
    ...
    for entry in records:
        ...
        if label == "<GOOD>":
            good_only_lines.append(f"<GOOD> {text}<|endoftext|>\n")
